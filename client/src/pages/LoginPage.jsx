...
justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
    ...
    p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        {currState}
                ...
                {!isDataSubmitted && currState === 'Sign up' && <input onChange={(e) => setFullName(e.target.value)} value={fullName} type="text" className='p-2 border border-gray-500 rounded-md focus:outline-none' placeholder="Full Name" required />}
                {!isDataSubmitted && <>
                ...
                {isDataSubmitted && <textarea onChange={(e) => setBio(e.target.value)} value={bio} rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='Provide a short bio...' required />}
...
to-violet-600 text-white rounded-md cursor-pointer'>
                ...
                {currState === 'Sign up' && <div className='flex items-center gap-2 text-sm text-gray-500'><input type="checkbox" required /><p>Agree to the terms of use & privacy policy.</p></div>}
